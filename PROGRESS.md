...
- Frontend web unit tests and clippy pass in an isolated Nix target directory (27 tests). The reviewed exact-head desktop/narrow household browser flow is now CI-gated through a disposable Compose PostgreSQL/backend/worker/frontend/Caddy/Mailpit stack; `make test-e2e-household` reproduces that narrow web gate. It does not claim Linux desktop, Android runtime, real-provider delivery, or full browser-suite acceptance.
