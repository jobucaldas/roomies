...
- Frontend web unit tests and clippy pass in an isolated Nix target directory (27 tests). The reviewed exact-head desktop/narrow core and household browser flows are CI-gated through one disposable Compose PostgreSQL/backend/worker/frontend/Caddy/Mailpit stack; `make test-e2e-household` reproduces that web gate. Core evidence is sanitized JSON only (no screenshots or traces). It does not claim Linux desktop, Android runtime, real-provider delivery, or full browser-suite acceptance.
